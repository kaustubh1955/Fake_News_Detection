...
def create_glove_dict(path_to_text:str): # 0.75 Marks
    ...
    Your code goes here. 
    '''
    f=open(path_to_text,'r',encoding='utf8')
    for line in f:
        lin=line.split(' ')
        key=lin[0]
        vallue=lin[1:]
        value=numpy.array(vallue)
        embeddings[key]=value.astype(numpy.float)
    return embeddings
...
def get_max_length(dataframe: pandas.DataFrame, column_number: int): # 0.75 Marks
    ...
    Your code goes here
    '''
    column_B = dataframe.iloc[:, column_number]
    for i in column_B:
        s = i.lower()
        l = nltk.tokenize.word_tokenize(s)
        ll=len(l)
