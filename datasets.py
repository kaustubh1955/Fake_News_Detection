...
class dataset(Dataset):
    ...
    def __getitem__(self, idx): # 1.25 Mark
        ...
        SMT=[]
        JTF=[]
        SR = self.dataframe.iloc[idx, 3]
        SL= SR.lower()
        SMT.extend(word_tokenize(SL))
        
        JR = self.dataframe.iloc[idx, 15]
        JL = JR.lower()
        JTF.extend(word_tokenize(JL))
        
        label = torch.tensor(self.labels[self.dataframe.iloc[idx, 2]])

        #step 3
        vectorized_statement=numpy.zeros([200, self.statement_max])
        vectorized_justification = numpy.zeros([200, self.justification_max])
        
        for pos, word in enumerate(SMT):
            try:
        ...
        vectorized_statement = torch.tensor(numpy.float32(vectorized_statement))
        vectorized_justification= torch.tensor(numpy.float32(vectorized_justification))
        
        credit_history = torch.tensor(numpy.float32(self.dataframe.iloc[idx, 9:14]))
